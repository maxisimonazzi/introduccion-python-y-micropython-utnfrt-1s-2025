...
boton_estados_previos = [1, 1, 1, 1]  # Estados previos (sin presionar)
...
def monitorear_botones():
    """Función que monitorea los botones físicos en un hilo separado"""
    global boton_estados_previos
    
    while True:
        for i in range(len(boton_pins)):
            estado_actual = boton_pins[i].value()
            
            # Detectar flanco descendente (botón presionado)
            if boton_estados_previos[i] == 1 and estado_actual == 0:
                # Toggle del LED correspondiente
                led_actual = led_pins[i].value()
                led_pins[i].value(1 - led_actual)  # Invertir estado
                
                estado_texto = "ON" if led_pins[i].value() else "OFF"
                print(f'Botón {i} presionado - LED {i} {estado_texto}')
                
                # Pequeña pausa para evitar rebotes
                time.sleep(0.05)
            
            boton_estados_previos[i] = estado_actual
...
def web_page():
    ...
    <head>
    <title>Servidor ESP UTN - Control Híbrido</title>
    ...
    <body>
        <h1>Servidor ESP - Control Híbrido LEDs</h1>
        <h2>Control por Web y Botones Físicos</h2>
        
        <div class="info-panel">
            <p><strong>Instrucciones:</strong></p>
            <p>• Usa los botones web o fisicos para controlar el sistema</p>
            <p>• La página se actualiza automáticamente cada 2 segundos</p>
        </div>

        <table class="tg"><thead>
            <tr>
                <th class="tg-0lax"><p>LED 3: <strong>""" + estado_led3 + """</strong></p></th>
                <th class="tg-0lax"><p>LED 2: <strong>""" + estado_led2 + """</strong></p></th>
                <th class="tg-0lax"><p>LED 1: <strong>""" + estado_led1 + """</strong></p></th>
                <th class="tg-0lax"><p>LED 0: <strong>""" + estado_led0 + """</strong></p></th>
            </tr></thead>
            <tbody>
            <tr>
...
def servidor_web(wlan):
    ...
    print('\nControl disponible:')
    print('- Botones web en la interfaz')
    print('- Botones físicos: Pin 15, 4, 5, 18 (toggle)')
    ...
    while True:
        try:
            ...
            # Procesar comandos de LEDs desde la web y redirigir
            comando_procesado = False
            
            if request.find('/?led0=on') != -1:
                led_pins[0].value(1)
                print('LED 0 ON (Web)')
                comando_procesado = True
            elif request.find('/?led0=off') != -1:
                led_pins[0].value(0)
                print('LED 0 OFF (Web)')
                comando_procesado = True
            elif request.find('/?led1=on') != -1:
                led_pins[1].value(1)
                print('LED 1 ON (Web)')
                comando_procesado = True
                ...
                print('LED 2 OFF (Web)')
                comando_procesado = True
            elif request.find('/?led3=on') != -1:
                led_pins[3].value(1)
                print('LED 3 ON (Web)')
                comando_procesado = True
            elif request.find('/?led3=off') != -1:
                led_pins[3].value(0)
                print('LED 3 OFF (Web)')
                comando_procesado = True

            # Si se procesó un comando, redirigir a la página principal
...
def main():
    ...
    print('Iniciando sistema de control híbrido...')
    
    wlan = conectar_wifi()
    if wlan:
        # Iniciar el hilo para monitorear botones físicos
        print('Iniciando monitoreo de botones físicos...')
        _thread.start_new_thread(monitorear_botones, ())
